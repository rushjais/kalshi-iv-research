...
\begin{itemize}
    \item \textbf{Regime-Based Timing Guidance:} Given the observed 2-day lead, these signals can be used to identify potential regime shifts (e.g., from ``Stable'' to ``Stressed'') prior to their reflection in the broader volatility surface.
    \item \textbf{Expectation Arbitrage:} The model identifies specific windows where $P_{Kalshi}$ and $P_{Options}$ diverge. This provides a framework to monitor the gap between retail expectations and institutional hedging flows.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Formally calculating the Arbitrage Coefficient ($\lambda$) to identify specific entry and exit points for volatility-based trades.
